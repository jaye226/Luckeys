...
#import "LKNotCodesData.h"

#define paySuccess 10005

@interface LKOrderPayViewPage () <UITableViewDataSource,UITableViewDelegate>
{
...
@implementation LKOrderPayViewPage

- (instancetype)init
{
    self = [super init];
    if (self) {
        [self registController:@"LKOrderPayController"];
    }
    return self;
}
...
- (void)initWithParam:(NSDictionary *)paramInfo
{
    ...
    {
        _selectData = [paramInfo objectForKey:@"data"];
        self.title= [paramInfo objectForKey:@"name"];
    }
}
...
- (void)viewDidLoad {
    ...
    _payModel = [self.controller getModelFromListByName:@"LKControllerModel"];
...
- (void)addHeadView
{
    ...
    _numberLabel.text = [NSString stringWithFormat:@"x%lu",(unsigned long)[_selectData.listCodes count]];
    ...
    _timeLabel.text = [NSString stringWithFormat:@"订单金额：%ld元", [_selectData.unitPrice integerValue]*[_selectData.listCodes count]];
    ...
    switch (_selectdPay) {
        ...
        case PayType_Ali:{
            if (_payModel.aliPayData) {
                _payModel.aliPayData.activityName = _selectData.activityName;
                _payModel.aliPayData.descri = _selectData.descri;
                LKPayManger *payManger = [LKPayManger sharedManager];
...
- (void)requestWXOrder
{
    ...
    for (LKNotCodesData *codeData in _selectData.listCodes)
    {
        [arry addObject:codeData.codeUuid];
    }
    [body setObject:_selectData.activityUuid forKey:@"activityUuid"];
    [body setObject:arry forKey:@"listCodeUuid"];
    [body setObject:[[LKShareUserInfo share] userInfo].userUuid forKey:@"userUuid"];
    [body setObject:_selectData.activityName forKey:@"activityName"];
    [body setObject:_selectData.descri forKey:@"description"];
    [body setObject:_selectData.unitPrice forKey:@"unitPrice"];
    [body setObject:@"Sign=WXPay" forKey:@"wxPackage"];
    [body setObject:[NSString stringWithFormat:@"%lu", (unsigned long)[_selectData.listCodes count]] forKey:@"bets"];
...
- (void)managerDidRecvPayResponse:(PayResp *)response{
    switch(response.errCode){
        case WXSuccess:
        {
            ...
            //            NSDictionary *body = @{@"prepayId":STR_IS_NULL(_payModel.payData.prepay_id),@"activityUuid":STR_IS_NULL(_selectData.activityUuid)};
