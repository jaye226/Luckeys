...
//static pthread_mutex_t  p_mutext;
//#define pthread_trylock(mutex)    if(!pthread_main_np()){ pthread_mutex_lock(&mutex);}
//#define pthread_tryunlock(mutex)  if(!pthread_main_np()){ pthread_mutex_unlock(&mutex);}

static NSRecursiveLock  *p_mutext = nil;
#define pthread_trylock(mutex)    //if(!pthread_main_np()){ [p_mutext lock];}
#define pthread_tryunlock(mutex)  //if(!pthread_main_np()){ [p_mutext unlock];}
...
+ (void)initialize
{
   [super initialize];
    ...
    dispatch_once(&onceToken, ^{
        p_mutext = [[NSRecursiveLock alloc] init];
    });
}
...
//				[oneItem release];
								...
								while (sqlite3_step(setStmt) == SQLITE_ROW)
								{
									...
									else
									{
										...
										if ([objectClass shouldBeStoredInBlob])
										{
                                            ...
                                            if (theObject)[set addObject:theObject];
										}
										else
										{
			...
			if (oneItem)[ret addObject:oneItem];
		}
		sqlite3_finalize(statement);
	}
	...
	if (!dirty)
	{
		...
		for (NSString *propName in props)
		{
			...
			if ([propType hasPrefix:@"@"] ) // Object
			{
				...
				if (! (isCollectionType(className)) )
				{
					if ([[theProperty class] isSubclassOfClass:[SQLitePersistentObject class]] && [theProperty isDirty])
                    {
                            dirty = YES;
                    }
				}
